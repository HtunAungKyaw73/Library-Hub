 ...
 */ async function baserowFetch(endpoint, options = {}) {
    const url = endpoint.startsWith("http") ? endpoint : `${BASEROW_API_URL}${endpoint}`;
...
async function getBooks() {
    const results = await baserowFetchAll(`/api/database/rows/table/${TABLE_BOOKS}/?user_field_names=true`);
...
async function getAuthors() {
    const results = await baserowFetchAll(`/api/database/rows/table/${TABLE_AUTHORS}/?user_field_names=true`);
    return results;
}
